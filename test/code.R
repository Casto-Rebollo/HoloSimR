...
cor_mat1 <- as.matrix(founderMxM)
colnames(mbiome.sym) <- colnames(data)
cor_mat2 <- cor(mbiome.sym)
corrplot(as.matrix(founderMxM), method = "color", order = "hclust", tl.pos="n")

corrplot(cor(mbiome.sym), method = "color", order = "hclust", tl.pos="n")

hc1 <- hclust(as.dist(1 - cor_mat1))
hc2 <- hclust(as.dist(1 - cor_mat2))

plot(hc1, main = "Clustering from Matrix 1")
plot(hc2, main = "Clustering from Matrix 2")
